...
//                date = date.replace(".", "")
//                        .replace(date.substring(date.length() - 2, date.length() - 1), date.substring(date.length() - 2, date.length() - 1).toUpperCase());
                if (i < 1) {
                    return getName() + " спит.";
                } else {
                    ...
                    return getName() + " лает г" + stringBuilder.toString() + "в! " + date;
                }
            }
        }
        return new DogPet();
